...
class SnippetControllerTest {
    ...
    @Test fun `registerSnippet returns 200`() {
        ...
        mockMvc.perform(
            put("/api/v1/snippets/snippet1")
                ...
                .content("""{"snippetId":"snippet1","language":"kotlin","ownerId":"$userId"}"""),
        )
            .andExpect(status().isOk)
    }
    ...
    @Test fun `removeSnippetPermission returns 200`() {
        setupJwt()
        mockMvc.perform(
            delete("/api/v1/snippets/snippet1/permission")
                .header("Authorization", "Bearer token"),
        )
            .andExpect(status().isOk)
    }
    ...
    @Test fun `runSnippet returns 200`() {
        setupJwt()
        mockMvc.perform(
            post("/api/v1/snippets/snippet1/run")
                .header("Authorization", "Bearer token")
                .contentType(MediaType.APPLICATION_JSON)
                .content("""{"input":"hello"}"""),
        )
    ...
    @Test fun `cancelSnippetExecution returns 200`() {
        setupJwt()
        mockMvc.perform(
            delete("/api/v1/snippets/snippet1/run")
                .header("Authorization", "Bearer token"),
        )
            .andExpect(status().isOk)
    }
